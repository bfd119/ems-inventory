...
## 6. Webアプリとして公開する場合（ブラウザから実行）

今回の更新で、ブラウザからURLにアクセスするだけでメール送信を実行できるようになりました（手動実行用）。

1. 右上の「デプロイ」 > 「新しいデプロイ」を選択。
2. 「種類の選択」の歯車アイコン > 「ウェブアプリ」を選択。
3. 説明: 「手動実行用」など。
4. 次のユーザーとして実行: 「自分 (`...@gmail.com`)」。
5. アクセスできるユーザー: **「自分のみ」** (セキュリティのため、基本は自分のみ推奨)。
6. 「デプロイ」をクリックし、発行されたURLをブックマークしておきます。
   - このURLにアクセスすると、メール送信処理が実行され「メール送信完了」画面が表示されます。

**重要: コードを更新した場合**
すでにデプロイ済みの場合は、以下の手順で更新を反映させてください。
1. 「デプロイ」 > 「デプロイを管理」を選択。
2. 左側の項目から「Webアプリ」の設定を選択し、鉛筆アイコン（編集）をクリック。
3. バージョン: **「新バージョン」** を選択。
4. 「デプロイ」をクリック。
※ これを行わないと、古いコードのまま（doGet関数がない状態）で動作してしまいます。
